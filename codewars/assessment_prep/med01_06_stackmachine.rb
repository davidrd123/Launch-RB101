...
- DIV & MOD are integer operations
- Programs can be assumed to be correct
...
- Initialize the register to 0
- Initialize stack to []
- Split the input string on spaces into commands
- While commands is not empty:
  - Shift off the next command
    - When integer, store in register (handle this separately)
    - When 'PUSH', push the register value onto the stack
    - WHen 'POP', remove the top item from the stack & put in register
    - When 'ADD'/ 'SUB'/MULT/DIV/MOD, pop a value from the stack, perform op on register, store result in register
  ...
  until commands.empty?
    ...
    case command
      ...
      when 'MULT'
        register *= stack.pop
      when 'DIV'
        register = register.divmod(stack.pop)[0]
      when 'MOD'
